...
const onUpdate = (id, param) => {
  category.put(id, param).then((res) => {
    if (res.status === 200) {
          ...
          <table class="w-full text-center text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            >
              <tr>
                ...
                <th scope="col" class="px-4 py-3">Mahsulotni turini o'chirish</th>
                <th scope="col" class="px-4 py-3">Mahsulotni turini o'zgartirish</th>
              </tr>
            </thead>
            <h1 v-if="!state.LIST.length" class="text-center font-semibold text-xl">
              Hozircha ma'lumot yo'q !!!
            </h1>
            <tbody v-else>
              <tr
                v-for="(el, ind) in state.LIST"
                :key="ind"
